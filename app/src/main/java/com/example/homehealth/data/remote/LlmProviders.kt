 ...
 *
 * ⚠️ 下面的 `visionModels` / `chatModels` 是**推荐清单，不是可选范围**。
 * 供应商型号更新频繁，这份硬编码清单必然滞后；设置页允许用户直接填写任意模型名，
 * 因此不要假设「不在清单里的模型不可用」——清单只用于下拉快捷填入与默认值回退。
 * （后续如要自动化，正确方向是调用各家的 `/v1/models` 动态获取，而不是继续手工维护。）
 *
 * 两类模型严格区分、独立配置：
