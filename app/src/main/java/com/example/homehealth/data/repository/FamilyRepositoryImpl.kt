...
class FamilyRepositoryImpl @Inject constructor(
     ...
     * 日历事件 ID 存在 `medication_reminders.calendarEventIds`、图片路径存在
     * `medical_documents.filePath` —— 一旦先删了行，这两类信息就再也拿不回来，
     * 结果是用户日历里留下永久的孤儿日程、应用私有目录里留下体检报告原图（隐私问题）。
     */
    override suspend fun deleteMember(member: FamilyMember) {
        ...
        // 4) 数据库：health_records 通过外键级联删除，其余表手动清理
